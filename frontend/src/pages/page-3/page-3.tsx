...
interface EventItem {
  ...
  category?: string;
  /*
   * Теги читаются фильтром ниже, но в описании их не было: тип написан руками
   * и разошёлся с контрактом. Компилятор молчал, потому что типы фронта не
   * проверялись вовсе, — фильтр работал только благодаря тому, что сервер поле
   * всё-таки отдаёт.
   */
  tags?: string[];
  image?: string | null;
