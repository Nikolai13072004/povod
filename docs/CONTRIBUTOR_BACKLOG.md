...
| BE-011 | P1 | DECISION | Реализовать избранные события отдельной таблицей и идемпотентными endpoints. |
...
| FE-006 | P1 | READY | Синхронизировать поиск и фильтры с URL query params, чтобы ссылкой можно было поделиться и восстановить состояние после reload. |
...
| ID | Приоритет | Режим | Задача и ожидаемый результат |
| --- | --- | --- | --- |
| PROD-001 | P1 | DECISION | Избранное: модель, API, кнопка, раздел пользователя и тесты. |
| PROD-002 | P1 | DECISION | Приглашения в приватное событие и управление списком приглашённых. |
| PROD-003 | P1 | DECISION | Настоящие уведомления о приглашении, изменении времени, отмене и новом комментарии. |
