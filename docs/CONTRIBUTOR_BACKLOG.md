...
| SEC-012 | P1 | DECISION | Дружба без согласия: `POST /api/Users/:id/friends` создаёт симметричную связь и меняет запись второй стороны, которая ни о чём не спрашивалась; `GET /api/Users` и `GET /api/Users/:id/friends` отвечают анониму. Нужна модель заявка/подтверждение (миграция), `requireAuth` на каталог и пагинация. |
