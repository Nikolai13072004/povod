...
| SEC-008 | P1 | DONE | Восстановление пароля: одноразовый токен на час, только SHA-256 в базе, одинаковый ответ для любого адреса, отзыв всех сессий после смены. Подтверждение email и подключение провайдера почты — осталось. |
