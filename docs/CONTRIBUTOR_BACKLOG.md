...
| SEC-013 | P1 | READY | Расход приглашения не в одной транзакции с записью: `resolveInvitation` читает счётчик, `joinEvent` пишет участника, `consumeInvitation` расходует лимит — и его результат игнорируется. Двое по ссылке с `maxUses=1` проходят оба; отзыв между шагами не отменяет уже совершённую запись. |
| SEC-014 | P1 | READY | KudaGo: `fetch` без таймаута, без дедупликации параллельных вызовов и без кэширования отказа. `GET /api/Events/:id` с неизвестным id триггерит внешний запрос, поэтому перебор идентификаторов даёт усиление наружу. |
